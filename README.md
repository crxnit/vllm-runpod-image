...
window.CHAT_CONFIG = {
  ...
  title: 'Advisor',              // header title
  titleAccent: 'My',             // accented portion of title
  subtitle: 'A helpful guide',   // subtitle text
...
All DOM elements (header, chat area, input, setup overlay, settings bar) are generated by `chat.js` based on the config — no HTML boilerplate to copy. Theming is via CSS variables (`--accent`, `--bg-primary`, etc.) — override in a `<style>` block for custom colors.
