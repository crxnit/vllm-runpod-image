...
- **Web UI** — `ui/index.html` (developer) and purpose-built UIs like `ui/college-advisor.html` (personalized for Kate, focused on UT Austin). All share `ui/shared/styles.css`, `ui/shared/markdown.js`, and `ui/shared/chat.js`. New UIs only need an HTML file with a `CHAT_CONFIG` object defining `id`, `mode` (simple/developer), `systemPrompt`, `welcomeMessage`, `starters`, `maxTokens`, `temperature`, `stripThinking`. `CHAT_CONFIG` also supports optional `endpoint` and `apikey` fields for pre-configured connections (used by the production proxy deployment). Endpoint validation rejects non-HTTPS URLs (except localhost) to prevent API key leakage over plaintext. Features: markdown rendering in assistant messages (code blocks, bold, italic, lists, links, headings), dark/light theme toggle, copy buttons on messages and code blocks, message retry, conversation export (.md), typing indicator with animated dots, smart auto-scroll. Theming via CSS custom properties with `[data-theme]` attribute. Settings and theme persist in localStorage.
