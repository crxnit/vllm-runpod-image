...
- **CLI Shared** — `cli/common.py` provides shared config loading (`~/.config/vllm-chat/config.json`), save, and ANSI color constants used by all CLI tools.
- **CLI Chat** — `cli/chat.py` is a workspace-aware terminal chat interface (requires `openai` pip package). Run from any project directory: `python cli/chat.py --endpoint URL --key KEY`. Architecture: `ChatContext` holds session state, `COMMANDS` dict maps slash commands to handler functions (open for extension), `ACTION_HANDLERS` registry maps XML tag patterns to action handlers. `ChatClient`/`OpenAIChatClient` abstract the API layer. `gather_workspace_context()` isolates side effects from the pure `build_system_prompt()` function. The model can autonomously read/write files and run commands using XML tags (`<write_file>`, `<read_file>`, `<run_command>`), with user confirmation for each action (toggle with `/auto`). Shell commands always require confirmation even in auto-approve mode. File paths are validated against a safe character pattern and must resolve within the workspace. Supports all workspace commands (`/read`, `/write`, `/ls`, `/tree`, `/diff`, `/sh`), `@file.py` inline file references, and standard chat commands. Streaming, multi-turn conversation, Ctrl+C to cancel.
...
- **Web UI** — `ui/index.html` (developer) and purpose-built UIs like `ui/college-advisor.html` (personalized for Kate, focused on UT Austin). All load `ui/shared/utils.js` (shared `escapeHtml`, `copyToClipboard`), `ui/shared/markdown.js`, and `ui/shared/chat.js`. Architecture: `layout` handles DOM construction, `dom` is a property bag of element references, `renderer` handles message display/status/scrolling, `sendChatRequest()` is a decoupled API layer accepting explicit parameters, and `MODE_INITIALIZERS` is a registry for extensible mode support. `CHAT_CONFIG` supports `id`, `mode` (simple/developer), `systemPrompt`, `welcomeMessage`, `starters`, `maxTokens`, `temperature`, `stripThinking`, and optional `endpoint`/`apikey` for pre-configured proxy connections. Endpoint validation rejects non-HTTPS URLs (except localhost). Features: markdown rendering, dark/light theme, copy buttons, message retry, conversation export (.md), typing indicator, smart auto-scroll. Settings persist in localStorage.
