...
- **CLI Chat** — `cli/chat.py` is a workspace-aware terminal chat interface (requires `openai` pip package). Config saved to `~/.config/vllm-chat/config.json`. Run from any project directory: `python cli/chat.py --endpoint URL --key KEY`. Automatically includes CWD, git branch, and file listing in system prompt. Supports `/read`, `/write`, `/ls`, `/tree`, `/diff`, `/sh` workspace commands, `@file.py` inline file references, and standard chat commands (`/help`, `/clear`, `/system`, `/temp`, `/max`, `/model`, `/history`). Streaming, multi-turn conversation, Ctrl+C to cancel.
...
- Recommended GPU for 3B AWQ models: RTX A4000, L4, or RTX A5000. 32B models need an A5000/4090 (24GB). 70B AWQ models need A100 80GB.
- Container disk: 20GB for 3B models, 40GB for 32B, 80GB for 70B.
